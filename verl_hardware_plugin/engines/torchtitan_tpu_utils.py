...
"""TPU sequence packing utilities, attention monkey patches, and memory layout helpers for TorchTitan."""
...
class TPUDocumentAttention(torch.nn.Module):
...
def replace_varlen_attention_with_tpu_attention(modules: Any) -> int:
    """Replaces inner_attention submodules on model instances with TPUDocumentAttention."""
    ...
    for root in model_list:
        ...
        for sub in root.modules():
            if hasattr(sub, "inner_attention") and not isinstance(sub.inner_attention, TPUDocumentAttention):
                sub.inner_attention = TPUDocumentAttention()
                replaced += 1
    return replaced
...
def _resolve_target_device(fallback_device: Any = "cpu") -> Any:
    """Resolve current accelerator device ID, falling back to CPU when no accelerator runtime is active."""
    from verl.utils.device import get_device_id, get_device_name

    try:
        dev_name = get_device_name()
        if dev_name == "tpu" and not hasattr(torch, "tpu"):
            return fallback_device
        dev_id = get_device_id()
        if dev_name == "tpu":
            return f"tpu:{dev_id}" if isinstance(dev_id, int) else dev_id
        return dev_id
    except Exception:
        return fallback_device
...
def safe_to_padded_tensor(nt: Any, padding: Any = 0, output_size: Any = None) -> torch.Tensor:
    ...
    if not getattr(nt, "is_nested", False):
        if isinstance(nt, torch.Tensor) and nt.device.type == "cpu":
            return nt.to(device=_resolve_target_device(nt.device))
        return nt
    target_device = _resolve_target_device(nt.device) if nt.device.type == "cpu" else nt.device
...
def select_and_to_padded_tensor(data: TensorDict, *fields: str) -> TensorDict:
    ...
    target_device = _resolve_target_device("cpu")
    padded_dict = {}
    for k in fields:
        if k in data.keys():
            val = data[k]
